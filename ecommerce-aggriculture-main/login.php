...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		...
		$result = mysqli_query($mysqlCon, "SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='yes'");
		...
		if ($num > 0) {
			...
			if (isset($_REQUEST['ono'])) {
				$ono = mysqli_real_escape_string($mysqlCon, $_REQUEST['ono']);
			...
			$result1 = mysqli_query($mysqlCon, "SELECT * FROM user WHERE (email='$user_login') AND password='$password_login_md5' AND activation='no'");
...
if (isset($_POST['activate'])) {
	if (isset($_POST['actcode'])) {
		$user_login = mysqli_real_escape_string($mysqlCon, $_POST['acemail']);
		$user_login = mb_convert_case($user_login, MB_CASE_LOWER, "UTF-8");
		$user_acccode = mysqli_real_escape_string($mysqlCon, $_POST['actcode']);
		$result2 = mysqli_query($mysqlCon, "SELECT * FROM user WHERE (email='$user_login') AND confirmCode='$user_acccode'");
		...
		if ($num3 > 0) {
			...
			$_SESSION['user_login'] = $get_user_uname_db;
			setcookie('user_login', $user_login, time() + (365 * 24 * 60 * 60), "/");
			mysqli_query($mysqlCon, "UPDATE user SET confirmCode='0', activation='yes' WHERE email='$user_login'");
			if (isset($_REQUEST['ono'])) {
				$ono = mysqli_real_escape_string($mysqlCon, $_REQUEST['ono']);
				header("location: orderform.php?poid=" . $ono . "");
			} else {
...
<body class="home-welcome-text" style="background: linear-gradient(rgba(0, 0, 0, .5), rgba(0, 0, 0, .5)), url(image/homebackgrndimg1.png);">
							...
							<form action="" method="POST" class="registration">
								<div class="signup_form">
									...
									if (isset($activacc)) {
										...
										<div>
											<td>
												<input name="email" autocomplete="off"  placeholder="Enter Your Email" required="required" class="email signupbox" type="email" size="30" value="' . $emails . '">
											</td>
										</div>
										<div>
											<td>
