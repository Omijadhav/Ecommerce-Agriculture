...
if (isset($_REQUEST['poid'])) {
	...
	$poid = mysqli_real_escape_string($mysqlCon, $_REQUEST['poid']);
} else {
...
$getposts = mysqli_query($mysqlCon, "SELECT * FROM products WHERE id ='$poid'");
...
if (isset($_POST['order'])) {
	...
	try {
		...
		if (mysqli_query($mysqlCon, "INSERT INTO orders (uid,pid,quantity,oplace,mobile,odate,ddate) VALUES ('$user','$poid',$quan,'$_POST[address]','$_POST[mobile]','$d','$date')")) {
...
<body style="background: linear-gradient(rgba(0, 0, 0, .5), rgba(0, 0, 0, .5)), url(image/homebackgrndimg1.png);">
			...
			<div>
				...
				<div style="float: right;">
						...
						<div>
							<form action="" method="POST" class="registration">
								<div class="signup_form" style="    margin-top: 38px;">
									<div>
										<td>
											<input autocomplete="off" name="mobile" placeholder="Your mobile number" required="required" class="email signupbox" type="text" size="30" value="' . $umob_db . '">
										</td>
									</div>
									<div>
										<td>
											<input autocomplete="off" name="address" id="password-1" required="required"  placeholder="Write your full address" class="password signupbox " type="text" size="30" value="' . $uadd_db . '">
										</td>
									</div>
									
									<div>
										<td>
											<select type="text" required="required" style=" font-size: 20px;
											font-style: italic; margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 0;width: 300px;background-color: transparent;" class="">
												<option default>cash on delievery</option>
					...
					<?php echo '
									...
									<div>
										<input autocomplete="off" name="order" class="uisignupbutton signupbutton" type="submit" value="Confirm Order">
									</div>
	...
	<hr>
	<center id="google_translate_element">
		</ce>
