...
if (isset($_REQUEST['uid'])) {
	...
	$user2 = mysqli_real_escape_string($mysqlCon, $_REQUEST['uid']);
	if ($user != $user2) {
	...
	try {
		...
		if (isset($opass) && isset($npass) && isset($npass1) && ($opass != "" && $npass != "" && $npass1 != "")) {
			if (md5($opass) == $upass) {
				if ($npass == $npass1) {
					$npass = md5($npass);
					mysqli_query($mysqlCon, "UPDATE user SET password='$npass' WHERE id='$user'");
					$success_message = '
		...
		if ($uemail_db != $email) {
			if (mysqli_query($mysqlCon, "UPDATE user SET  email='$email' WHERE id='$user'")) {
				//success message
				$success_message = '
					<div class="signupform_text" style="font-size: 18px; text-align: center;">
					<font face="bookman">
					...
					<div class="holecontainer" style=" padding-top: 20px; padding: 0 20%">
						<form action="" method="POST" class="registration">
							<div class="container signupform_content ">
								...
								<div>
									<td><input autocomplete="off" class="email signupbox" type="password" name="opass" placeholder="Old Password"></td>
								</div>
								<div>
									<td><input autocomplete="off" class="email signupbox" type="password" name="npass" placeholder="New Password"></td>
								</div>
								<div>
									<td><input autocomplete="off" class="email signupbox" type="password" name="npass1" placeholder="Repeat Password"></td>
								</div>
								<div style="text-align: center;font-size: 20px;color: #fff;margin: 0 0 5px 0;">
									<td>Change Email:</td>
								</div>
								<div>
									<td><?php echo '<input autocomplete="off" class="email signupbox" required type="email" name="email" placeholder="New Email" value="' . $uemail_db . '">'; ?></td>
								</div>
								<div>
									<td><input autocomplete="off" class="uisignupbutton signupbutton" type="submit" name="changesettings" value="Update Settings"></td>
								</div>
								<div>
