...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['epid'])) {
		...
		$epid = mysqli_real_escape_string($mysqlCon, $_REQUEST['epid']);
	} else {
...
$getposts = mysqli_query($mysqlCon, "SELECT * FROM products WHERE id ='$epid'");
...
if (isset($_POST['updatepro'])) {
	...
	if ($result = mysqli_query($mysqlCon, "UPDATE products SET pName='$_POST[pname]',price='$_POST[price]',description='$_POST[descri]',available='$_POST[available]',category='$_POST[category]',type='$_POST[type]',item='$_POST[item]',pCode='$_POST[code]' WHERE id='$epid'")) {
		...
		if (((@$_FILES['profilepic']['type'] == 'image/jpeg') || (@$_FILES['profilepic']['type'] == 'image/png') || (@$_FILES['profilepic']['type'] == 'image/jpg') || (@$_FILES['profilepic']['type'] == 'image/gif')) && (@$_FILES['profilepic']['size'] < 1000000)) {
			...
			if (file_exists("../image/product/$item/" . $filename)) {
				...
				if (move_uploaded_file(@$_FILES["profilepic"]["tmp_name"], "../image/product/$item/" . $filename)) {
					$photos = $filename;
					if ($result = mysqli_query($mysqlCon, "UPDATE products SET picture='$photos' WHERE id='$epid'")) {
...
if (isset($_POST['delprod'])) {
	//triming name
	$getposts1 = mysqli_query($mysqlCon, "SELECT pid FROM orders WHERE pid='$epid'");
	if ($ttl = mysqli_num_rows($getposts1)) {
		$error_message = "You can not delete this product.<br>Someone ordered this.";
	} else {
		if (mysqli_query($mysqlCon, "DELETE FROM products WHERE id='$epid'")) {
...
<body style="background: linear-gradient(rgba(0, 0, 0, .7), rgba(0, 0, 0, .7)), url(../image/homebackgrndimg1.png);">
						...
						<div>
							<form action="" method="POST" class="registration">
								<div class="signup_form">
									<div>
										<td >
											<input autocomplete="off" name="pname" id="first_name" placeholder="Product Name" required="required" class="first_name signupbox" type="text" size="30" value="' . $pName . '" >
										</td>
									</div>
									<div>
										<td >
											<input autocomplete="off" name="price" id="last_name" placeholder="Price" required="required" class="last_name signupbox" type="text" size="30" value="' . $price . '" >
										</td>
									</div>
									<div>
										<td>
											<input autocomplete="off" name="available" placeholder="Available Quantity" required="required" class="email signupbox" type="text" size="30" value="' . $available . '">
										</td>
									</div>
									<div>
										<td >
											<input autocomplete="off" name="descri" id="first_name" placeholder="Description" required="required" class="first_name signupbox" type="text" size="30" value="' . $description . '" >
										</td>
									</div>
									<div>
										<td>
											<select hidden name="category" required="required" style=" font-size: 20px;
									...
									<div>
										<select name="type" hidden required="required" style=" font-size: 20px;
										font-style: italic;margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 0;width: 300px;background-color: transparent;" class="">
									...
									<div>
										...
										font-style: italic;margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 0;width: 300px;background-color: transparent;" class="">
												...
												<option selected value="herbicides">Herbicides</option>
									...
									<div>
										<td>
											<input autocomplete="off" name="code" hidden value="0" id="password-1" required="required"  placeholder="Code" class="password signupbox " type="text" size="30" value="' . $code . '">
										</td>
									</div>
									<div>
										<input autocomplete="off" name="updatepro" class="uisignupbutton signupbutton" type="submit" value="Update Product">
									</div>
									<div>
										<input autocomplete="off" name="delprod" class="uisignupbutton signupbutton" type="submit" value="Delete This Product">
									</div>
							...
							<li>
								<form action="" method="POST" class="registration" enctype="multipart/form-data">
										<div class="signup_form">
											<div>
												<td>
													<input autocomplete="off" name="profilepic" style="width: 115px;" class="password signupbox" type="file" value="Add Picture">
												</td>
											</div>
											<div>
												<input autocomplete="off" name="updatepic" style="width: 144px;" class="uisignupbutton signupbutton" type="submit" value="Change Picture">
											</div>
											<div class="signup_error_msg">';
