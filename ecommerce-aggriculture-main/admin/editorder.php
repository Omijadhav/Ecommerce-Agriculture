...
if (!isset($_SESSION['admin_login'])) {
	...
	if (isset($_REQUEST['eoid'])) {
		...
		$eoid = mysqli_real_escape_string($mysqlCon, $_REQUEST['eoid']);
		$getposts5 = mysqli_query($mysqlCon, "SELECT * FROM orders WHERE id='$eoid'");
	...
	$result1 = mysqli_query($mysqlCon, "SELECT * FROM orders WHERE id='$eoid'");
	...
	$result2 = mysqli_query($mysqlCon, "SELECT * FROM user WHERE id='$eouid'");
...
$getposts = mysqli_query($mysqlCon, "SELECT * FROM products WHERE id ='$eopid'");
...
if (isset($_POST['order'])) {
	...
	try {
		...
		if (mysqli_query($mysqlCon, "UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")) {
			//success message
...
if (isset($_POST['delorder'])) {
	//triming name
	if (mysqli_query($mysqlCon, "DELETE FROM orders WHERE id='$eoid'")) {
			...
			<div>
				...
				<div style="float: right;">
						...
						<div>
							<form action="" method="POST" class="registration">
								<div class="signup_form" style="    margin-top: 38px;">
									<div>
										<td>
											<input autocomplete="off" name="ddate" placeholder="Delevary date" required="required" class="email signupbox" type="date" size="30" value="' . $eddate . '">
										</td>
									</div>
									<div>
										<td>
					...
					<?php echo '
									...
									<div>
										<input autocomplete="off" name="order" class="uisignupbutton signupbutton" type="submit" value="Confirm Change">
									</div>
									<div>
										<input autocomplete="off" name="delorder" class="uisignupbutton signupbutton" type="submit" value="Delete Order">
									</div>
